...
if (isset($_POST['latitude']) && isset($_POST['longitude'])) {
    ...
    // Remove as coordenadas no banco de dados
    $sql = "DELETE FROM info WHERE latitude = ? AND longitude = ?";
    $stmt = $conexao->prepare($sql);
    $stmt->bind_param("dd", $latitude, $longitude);

    if ($stmt->execute()) {
        echo "Marcador removido com sucesso!";
    } else {
        echo "Erro ao remover o marcador: " . $stmt->error;
    }
